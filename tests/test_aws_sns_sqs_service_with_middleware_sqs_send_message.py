...
def test_start_aws_sns_sqs_service_with_middleware(capsys: Any, loop: Any) -> None:
    services, future = start_service(
        "tests/services/aws_sns_sqs_service_with_middleware_sqs_send_message.py", loop=loop
    )
    ...
    async def _async(loop: Any) -> None:
        ...
        while loop_until > time.time():
            if instance.test_queue_data_received:
                break
            await asyncio.sleep(0.5)

        assert instance.test_queue_data_received
        assert instance.test_topic_name == ""
        ...
        assert instance.test_message_attributes["traceparent"].startswith("00-")
