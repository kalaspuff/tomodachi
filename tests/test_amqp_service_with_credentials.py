...
@pytest.mark.skipif(not os.environ.get('TOMODACHI_TEST_RABBITMQ_ENABLED'), reason='RabbitMQ needs to be enabled and environment TOMODACHI_TEST_RABBITMQ_ENABLED needs to be set')
def test_start_amqp_service_with_credentials_with_custom_protocol(monkeypatch: Any, capsys: Any, loop: Any) -> None:
    services, future = start_service('tests/services/amqp_service_with_credentials_with_custom_protocol.py', monkeypatch)
