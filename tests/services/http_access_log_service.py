...
class HttpService(tomodachi.Service):
    ...
    @http('POST', r'/test_ignore_one', ignore_log=True)
    async def test_ignore_all(self, request: web.Request) -> str:
        return 'test'

    @http('POST', r'/test_ignore_one', ignore_log=[200])
    async def test_ignore_one(self, request: web.Request) -> tomodachi.HttpResponse:
        test = await request.text()
        if test == '200':
            return tomodachi.HttpResponse(body='test', status=200)
        else:
            return tomodachi.HttpResponse(body='test', status=201)
