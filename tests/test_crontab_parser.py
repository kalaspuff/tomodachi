...
def test_timezones() -> None:
    ...
    assert get_next_datetime("* * * * *", t) == pytz.timezone("Europe/Stockholm").localize(
        datetime.datetime(2017, 6, 15, 10, 17)
    )
    assert get_next_datetime("15 */2 * * *", t) == pytz.timezone("Europe/Stockholm").localize(
        datetime.datetime(2017, 6, 15, 12, 15)
    )
