...
- Adds instrumentation for ``opentelemetry`` that can be enabled if ``tomodachi`` is installed using the ``opentelemetry`` extras. Auto instrumentation using ``opentelemetry`` can then be activated by starting services using either ``tomodachi run --opentelemetry-instrument [options] <service.py ...>``, ``TOMODACHI_OPENTELEMETRY_INSTRUMENT=1 tomodachi run [options] <service.py ...>`` or ``opentelemetry-instrument tomodachi run [options] <service.py ...>``.
