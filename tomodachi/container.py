                            ...
                            for i, k_lvl in enumerate(key_split):
                                if i + 1 == len(key_split):
                                    if k_lvl in op_lvl and op_lvl[k_lvl] != context_options[key]:
                                        raise Exception("Missmatching options for '{}': ({}) \"{}\" and ({}) \"{}\" differs".format(key, type(context_options[key]).__name__, context_options[key], type(op_lvl[k_lvl]).__name__, op_lvl[k_lvl]))
